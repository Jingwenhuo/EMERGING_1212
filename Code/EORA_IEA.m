IEA_CO2=xlsread('/Users/jingwen/OneDrive/EMERGING_CO2/IEA_CO2_2015.xlsx','data_Eora');
...
X_=zeros(189,26);
for i=1:189 %用全部的对i国的能源投入看 竞争表
    X_(i,:)=sum(Z(2:26:end,(i-1)*26+1:i*26),1);
end            
X=zeros(189*26,1);
for i=1:189
   X((i-1)*26+1:i*26,:)=X_(i,:)';
end

X_ratio_map=xlsread('/Users/jingwen/OneDrive/EMERGING_CO2/input_102_105.xlsx','map_S_Eora'); %26
X_ratio_map=[X_ratio_map(:,1),X_ratio_map(:,3)];


IEA_Sector_matrix=xlsread('/Users/jingwen/OneDrive/EMERGING_CO2/IEA_CO2_2015.xlsx','S_map_Eora');
IEA_Sector_matrix=IEA_Sector_matrix(3:end,:);

IEA_Region_map=xlsread('/Users/jingwen/OneDrive/EMERGING_CO2/IEA_CO2_2015.xlsx','R_map_Eora'); %分了5大洲 需要减去有数据的国家
IEA_Region_map=[IEA_Region_map(:,1),IEA_Region_map(:,5)];
%%
% 整理IEA的数据
CO2_IEA=zeros(189*26,7); 
IEA_7=zeros(189*31,7);
%单个国家
for i=1:189
    ...
    if length(IEA_i)==0
      ...
      for j=1:7
         ...
         X_i_j=X((i-1)*26+1:i*26,X_ratio_map(j,2));
         X_i=sum(X((i-1)*26+1:i*26,:),2);
         X_T_i=X_T((i-1)*26+1:i*26,1);
         result=zeros(26,1);
         for k=1:31
         ...
         CO2_IEA((i-1)*26+1:i*26,j)=result;
      end
    end
end
%% 区域拆分 5个大区域 南极洲为0  最后用world 算出ROW
...
for i=-5:-1
    ...
    if length(IEA_R)==0
      ...
      for g=1:length(R_i)
          ...
          if length(IEA_i)==0
              E_total(g,1)=sum(sum(X((R_i(g)-1)*26+1:R_i(g)*26,:)));
              continue;
          else
      ...
      for h=1:length(Index_R_C)
         T= sum(sum(CO2_IEA((Index_R_C(h)-1)*26+1:Index_R_C(h)*26,:)));
         if T ~=0
             continue;
         else
         ...
         for j=1:7
          if j==5
           ...
           X_i_j=X((Index_R_C(h)-1)*26+1:Index_R_C(h)*26,X_ratio_map(j,2));
           X_i=sum(X((Index_R_C(h)-1)*26+1:Index_R_C(h)*26,:),2);
           X_T_i=sum(X_T((Index_R_C(h)-1)*26+1:Index_R_C(h)*26,:),2);
           result=zeros(26,1);
           for k=1:31
              S_I=IEA_Sector_matrix(find(IEA_Sector_matrix(:,k+1)==1),1);
              ratio=X_i_j(S_I,1)./sum(X_i_j(S_I,1));
           ...
           CO2_IEA((Index_R_C(h)-1)*26+1:Index_R_C(h)*26,j)=result;
         end
         end
      end
    end
